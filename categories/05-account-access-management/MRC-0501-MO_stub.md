...
tool: "Active Directory, PowerShell (AD Audit Script), ePO System Tree (cross-reference)"
jsig_controls:
  - AC-2
  - AC-6
  - IA-4
...
A PowerShell script queries Active Directory for all accounts whose `LastLogonDate` is beyond the 60-day (standard user) or 90-day (service account) threshold. The SA reviews the output, cross-references against the authorized user list, and disables accounts not justified by ISSM/ISSO.

2. **Privileged Group and Account Review:**
All members of Domain Admins, Enterprise Admins, local Administrators, and any other privileged groups are enumerated and compared to the ISSM-authorized privileged access list. Any account in a privileged group not on the authorized list is an AC-6 violation.
...
| Item | Details |
|------|---------|
| Active Directory (admin access) | ADUC, Active Directory Module for PowerShell |
| `Invoke-ADAccountAudit.ps1` | Library script — `scripts/Invoke-ADAccountAudit.ps1` |
...
|------|--------|--------------------|-----------------|
| 1 | Run account audit script | `.\Invoke-ADAccountAudit.ps1 -InactiveThresholdDays 60` | Script outputs: all accounts with LastLogonDate > 60 days, CSV export |
| 2 | Review output — identify standard user accounts (60-day threshold) | CSV output → filter AccountType: User | List of inactive standard users |
...
| 4 | Run script for service accounts (90-day threshold) | `.\Invoke-ADAccountAudit.ps1 -InactiveThresholdDays 90 -AccountType ServiceAccount` | List of inactive service accounts |
...
| 6 | Disable all accounts confirmed for disable | `Disable-ADAccount -Identity [samAccountName]` | Account disabled — do NOT delete; retain for AU-9 audit trail |
...
| Step | Action | Nav Path / Command | Expected Result |
|------|--------|--------------------|-----------------|
| 8 | Enumerate all members of Domain Admins | `Get-ADGroupMember -Identity "Domain Admins" -Recursive` | Full member list |
| 9 | Enumerate all members of Enterprise Admins | `Get-ADGroupMember -Identity "Enterprise Admins" -Recursive` | Full member list |
| 10 | Enumerate all members of local Administrators on all hosts | `Invoke-Command -ComputerName [all hosts] -ScriptBlock { net localgroup administrators }` | Local admin list per host |
| 11 | Enumerate all members of other privileged groups | Per local privileged group inventory (Backup Operators, Schema Admins, etc.) | Full list per group |
...
| 14 | Verify all privileged accounts have MFA / CAC enforced | AD → Account Properties → Smart Card Required | All privileged accounts: Smart Card required checked |

### Phase 3 — Administrative Account Document Review

| Step | Action | Nav Path / Command | Expected Result |
|------|--------|--------------------|-----------------|
| 15 | Pull administrative account list | `Get-ADUser -Filter {AdminCount -eq 1} \| Select Name, SamAccountName, Enabled, LastLogonDate` | All admin accounts listed |
...
| 17 | For each admin account: verify no anomalous activity in last 30 days | SIEM query: `index=windows EventCode=4624 OR 4625 OR 4648 user=[admin account]` | Activity consistent with authorized role; no off-hours or unexpected source logons |
